...
<form action="/doors">
    <div class="w-full mb-8 flex gap-2">
        ...
        @foreach($classifications as $classification)
            ...
            <option value="{{ $category->id }}" {{ isset($activeFilters['category']) && is_array($activeFilters['category']) ? (in_array($category->id, $activeFilters['category']) ? 'selected' : '') : '' }}>{{  ucfirst($category->name) }}</option>
            @endforeach
            </optgroup>
        @endforeach
        </select>
...
<script>
  ...
  Alpine.data("multiselect", () => ({
    style: {
      ...
      select:
        "border w-full border-gray-300 rounded-lg py-1 px-2 text-sm flex gap-2 items-center cursor-pointer bg-white",
      menuWrapper:
    ...
    init() {
      ...
      search.setAttribute("placeholder", "Search Door Categories");
      ...
      Alpine.data("newSelect", () => ({
        ...
        regenerateTextItems() {
          ...
          if (this.selectedItems.length === 0) {
            textList.innerHTML = '<span class="text-neutral-400 pl-2"><i class="fa-solid fa-filter"></i> Filter</span>';
          } else {
