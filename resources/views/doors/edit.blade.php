            ...
            <form method="POST" action="{{ route('updateDoor', $door->id) }}" enctype="multipart/form-data">
                ...
                <div class="mb-6">
                    ...
                    <input
                        ...
                        value="{{ old('name') ? old('name') : ($door->name ? $door->name : '') }}"
                    />
                    @error('name')
                ...
                <div class="mb-6">
                    ...
                    <input
                        ...
                        value="{{ old('sku') ? old('sku') : ($door->sku ? $door->sku : '') }}"
                    />
                    @error('sku')
                        <p class="text-red-500 text-xs mt-1">{{$message}}</p>
                    @enderror
                ...
                <div class="mb-6">
                  <label for="categories" class="inline-block text-lg text-gray-700 mb-2"
                    >Categories</label
                  >
                  @php
                    $currentDoorCategories = array();
                    foreach($door->categories as $doorCategory) { 
                      array_push($currentDoorCategories, $doorCategory['id']); 
                    } 
                  @endphp
                    <select id="categories" name="categories[]" multiple x-data="multiselect">
                      @foreach($classifications as $classification)
                        <optgroup label="{{ $classification->name }}">
                          ...
                          <option value="{{ $category->id }}" {{ (is_array(old('categories')) && in_array($category->id, old('categories'))) ? 'selected' : (!is_array(old('categories')) && in_array($category->id, $currentDoorCategories) ? 'selected' : '') }}>{{  ucfirst($category->name) }}</option>
                          @endforeach
                        </optgroup>
                      @endforeach
                    </select>
                ...
                <div class="mb-8 h-64">
                  ...
                  >
                  @php
                    $currentDoorPromotions = array();
                    foreach($door->promotions as $doorPromotion) { 
                      array_push($currentDoorPromotions, $doorPromotion['id']); 
                    } 
                  @endphp
                    <select id="promotions" name="promotions[]" multiple x-data="multiselectPromotions">
                      @foreach ($dbPromotionList as $promotion)
                        <option value="{{ $promotion->id }}" {{ (is_array(old('promotions')) && in_array($promotion->id, old('promotions'))) ? 'selected' : (!is_array(old('promotions')) && in_array($promotion->id, $currentDoorPromotions) ? 'selected' : '') }}>{{  ucfirst($promotion->name) }}</option>
                      @endforeach
                    </select>
                  @error('promotions')
                      <p class="text-red-500 text-xs mt-1">{{$message}}</p>
                ...
                <div class="mb-6 flex flex-col justify-center">
                  ...
                  <form method="POST" action="{{ route('destroyDoor', $door) }}">
                    ...
                    @method("DELETE")
                    <button 
                      class="text-white font-normal rounded bg-red-500 hover:bg-red-600 px-4 py-2 hover:cursor-pointer transition ease-out duration-200 mt-12"
